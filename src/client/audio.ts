...
export class RoomAudioEngine {
  ...
  handleSnapshot(snapshot: GameSnapshot | null, bounds: WindowBoundsPayload | null): void {
    const previousSnapshot = this.previousSnapshot
    this.previousSnapshot = snapshot

    if (!previousSnapshot || !snapshot || !bounds || !this.isReady() || document.hidden) {
      return
    }

    this.handleWallBounce(previousSnapshot, snapshot, bounds)
    this.handleBarrierBreak(previousSnapshot, snapshot, bounds)
    this.handleBonusCollect(previousSnapshot, snapshot, bounds)
    this.handleRouteCompletion(previousSnapshot, snapshot, bounds)
  }
  ...
  private handleWallBounce(
    ...
    const previousBall = this.findOwnedBall(previousSnapshot, bounds)
    const nextBall = this.findOwnedBall(nextSnapshot, bounds)
  ...
  private handleBarrierBreak(
    ...
    const barrierBroken = previousObstacles.some((previousObstacle) => {
  ...
  private findOwnedBall(snapshot: GameSnapshot, bounds: WindowBoundsPayload): BallState | null {
    const windowRect = rectFromWindow(bounds)

    return snapshot.balls.find((ball) =>
      ball.ownerWindowId === bounds.id && pointInRect(ball.x, ball.y, windowRect),
    ) ?? null
  }
